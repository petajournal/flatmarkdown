...
**Markdownリンク → Wikiリンク変換**: パース後、URL が `../ページパス.md`（または `../ページパス.md#id`）のパターンに一致する `link` ノードは `wikilink` ノードへ変換される。URL は `../` プレフィックスと `.md` サフィックスを除去した形に変換され、`#id` フラグメントはそのまま保持される。これにより、WikiリンクをMarkdownリンクとしてシリアライズしたMarkdownファイルを正しくラウンドトリップできる。パターンに一致しないリンク（外部URL・`.md`以外の相対パスなど）は `link` ノードのまま保持される。
